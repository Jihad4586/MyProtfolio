...
// Active Nav Link + Smooth Scroll (One-Page Sections)
const sectionEls = document.querySelectorAll(".section-block[id]");
const navAnchors = document.querySelectorAll(".nav-links a[data-section]");
const isOnePage = sectionEls.length > 0 && navAnchors.length > 0;
...
if (isOnePage) {
  // Smooth scroll + close mobile menu + update URL hash on click
  ...
  // Scroll-spy: highlight nav link for whichever section is in view
  ...
  // On load, jump to the section in the URL hash (if any) without animation
  if (location.hash) {
  ...
  // Standalone pages (e.g. resume.html): highlight by matching filename
